...
function NavList() {
  ...
  const navOps = Object.keys(urls).map((page) => {
    ...
    if (urls[page] === pathname) {
      return (
        <li key={page} className="nav-item ms-2">
          <a className={`${styles.customNavLink} ${styles.inactive} nav-link`} href="#">
            {page}
          </a>
        </li>
      );
    ...
    // All other pages
      ...
      return (
        <li key={page} className="nav-item ms-2">
          <a className={`${styles.customNavLink} nav-link`} href={urls[page]}>
            {page}
          </a>
        </li>
      );
      ...
      <ul className="navbar-nav w-100">
        ...
        {pathname !== '/login' ? (
          <li className="nav-item ms-auto">
            <a className={`${styles.customNavLink} nav-link`} href="#" onClick={handleLogout}>
              Logout
            </a>
          </li>
        ) : (
